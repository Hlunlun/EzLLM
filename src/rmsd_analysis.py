...
import pandas as pd
import os
from plot import plot_conditional_tmscores, plot_conditional_rmsd, plot_conditional_sim
import argparse
import difflib
...
def main():
    ...
    parser.add_argument('--top-path', type=str, default='projects/evodiff/data')
    ...
    home = os.path.join(pathlib.Path.home(), args.top_path, f'{args.model_type}/{args.pdb}')
    
    if not os.path.exists(os.path.join(home,'plots')):
        os.mkdir(os.path.join(home,'plots'))
    ...
    # with open(os.path.join(home, 'successes.csv'), 'w') as f:
    #     f.write(str(len(candidates)) + " of " + str(args.num_seqs) + " total")
    # f.close()
    success_path = os.path.join(home, 'success')
    os.makedirs(success_path, exist_ok=True)
    gen_seq_path = os.path.join(home, 'generated_samples_string.fasta')
    gen_seqs = list(SeqIO.parse(gen_seq_path, 'fasta'))
    candidates_seqs = [seq.replace("'",'').replace('[', '').replace(']', '') for seq in candidates['seqs'].tolist()]
    success_seqs = [record for record in gen_seqs if str(record.seq) in candidates_seqs ]
    with open(os.path.join(success_path, 'successes.fasta'), 'w') as f:
        SeqIO.write(success_seqs, f, 'fasta')
    candidates.to_csv(os.path.join(success_path, 'successes.csv'), index=True)
    motif_df.to_csv(os.path.join(home, 'motif_df_rmsd.csv'), index=True)

    # Eval TM scores
    tm = pd.read_csv(os.path.join(home, 'pdb/tmscores.txt'), names=['tmscore'])
    plot_conditional_tmscores(tm, ['grey'], legend=False, save_path=os.path.join(home,'plots', args.pdb))

    # Plot rmsd vs plddt
    plot_conditional_rmsd(args.pdb, motif_df, out_path=os.path.join(home, 'plots'))

    # percent similarity in fixed region
    chain_ids=[args.chain]
    structure = esm.inverse_folding.util.load_structure(os.path.join(home,'pdb', ref_pdb), chain_ids)
    ...
    with open(os.path.join(home, 'pdb/sim.txt'), 'w') as f:
        [f.write(str(s) + '\n') for s in sim]
    f.close()
    plot_conditional_sim(sim, out_path=os.path.join(home, f'plots/{args.pdb}'))
...
def calc_rmsd(num_structures, reference_PDB, fpath='conda/gen/6exz', ref_motif_starts=[30], ref_motif_ends=[44]):
    ...
    motif_df = pd.read_csv(os.path.join(fpath, 'motif_df.csv'), index_col=0, nrows=num_structures)
    rmsds = []
    for i in range(num_structures): # This needs to be in numerical order to match new_starts file
        ref = mda.Universe(os.path.join(fpath,'pdb',reference_PDB))
        u = mda.Universe(os.path.join(fpath, f'pdb/SEQUENCE_{i}.pdb'))
...
def get_confidence_score(fpath, num_structures, motif_df):
    ...
    for i in range(num_structures):
        new_motif_starts = literal_eval(motif_df['start_idxs'].iloc[i])[0]
        new_motif_ends = literal_eval(motif_df['end_idxs'].iloc[i])[-1]
        f = os.path.join(fpath, f'pdb/SEQUENCE_{i}.pdb')
        # Get pdb file number
        p = PDBParser()
        ...
        for i, res in enumerate(structure.get_residues()):
            for atom in res:
                scores_list.append(atom.bfactor) # 就是溫度因子化學元素前面那個數字就是
